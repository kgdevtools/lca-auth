...
import { createPuzzleLesson, createStudyLesson, createInteractiveStudyLesson, fetchStudentsForAssignment } from "./actions";
...
const BATCH_THEMES = [
  ...
  { group: 'Tactics', options: [
    { value: 'fork', label: 'Fork' }, { value: 'pin', label: 'Pin' },
    { value: 'skewer', label: 'Skewer' }, { value: 'discoveredAttack', label: 'Discovered Attack' },
    { value: 'doubleCheck', label: 'Double Check' }, { value: 'deflection', label: 'Deflection' },
    { value: 'hangingPiece', label: 'Hanging Piece' }, { value: 'trappedPiece', label: 'Trapped Piece' },
  ]},
  { group: 'Mates', options: [
    { value: 'mateIn1', label: 'Mate in 1' }, { value: 'mateIn2', label: 'Mate in 2' },
    { value: 'mateIn3', label: 'Mate in 3' }, { value: 'backRankMate', label: 'Back Rank Mate' },
    { value: 'smotheredMate', label: 'Smothered Mate' }, { value: 'arabianMate', label: 'Arabian Mate' },
  ]},
  { group: 'Endgame', options: [
    { value: 'endgame', label: 'Endgame' }, { value: 'pawnEndgame', label: 'Pawn Endgame' },
    ...
    <div className="rounded-lg border border-border bg-card overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          {!open && preview && (
            <span className="text-sm text-foreground/70 truncate">{preview}</span>
          )}
        </div>
        <ChevronDown
...
function LessonInfoForm({
  ...
  lessonInfo: {
    ...
    categoryId: string; difficulty: string;
    estimatedDurationMinutes: string; tags: string[];
  };
        ...
        {lessonInfo.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 pt-1">
            {lessonInfo.tags.map(tag => (
              ...
              >
                {tag}
                <button onClick={() => onRemoveTag(tag)} className="hover:text-foreground transition-colors">
                  <X className="w-3 h-3" />
                </button>
              </span>
            ))}
...
function SuccessBanner({ lessonId, label, onCreateAnother }: { lessonId: string; label: string; onCreateAnother: () => void }) {
  return (
    <div className="flex items-center gap-3 p-4 rounded-lg border border-border bg-card flex-wrap">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium">{label} created</p>
        <p className="text-xs text-muted-foreground">Saved as draft — publish when ready.</p>
      </div>
      <div className="flex gap-2">
  ...
  return (
    <Button onClick={onClick} disabled={isSubmitting} className="w-full">
      {isSubmitting ? (
        ...
        <><Check className="w-4 h-4 mr-2" /> Create lesson</>
      )}
    </Button>
  );
}
...
export default function AddLessonPage() {
  ...
  const [batchTheme, setBatchTheme]         = useState('caroKann');
  ...
  // Interactive study solve points (keyed by chapter id)
  ...
  const [lessonInfo, setLessonInfo] = useState({
    ...
    estimatedDurationMinutes: "", tags: [] as string[],
  });
  ...
  const handleCreateAnother = () => {
    // Reset all form fields so the creator is blank for a new lesson
    setPuzzles([]);
    setEditingPuzzleId(null);
    setEditingOrientation(undefined);
    ...
    setPuzzleHintInput("");
    setPuzzlePgnText("");
    setPuzzlePgnFenHistory([]);
    setPuzzlePgnMoveHistory([]);
    setPuzzlePgnPlyIndex(0);
    setChapters([]);
    setChapterNameInput("");
    setPgnInput("");
  ...
  const handleParsePuzzlePgn = () => {
    ...
    if (!raw) return;
    try {
      // loadPgn handles both full PGN (with headers) and bare move-text (Lichess game.pgn format)
      ...
      setPuzzlePgnFenHistory(fenHistory);
      setPuzzlePgnMoveHistory(moves);
      setPuzzlePgnPlyIndex(lastPly);
      handleFenChange(fenHistory[lastPly]);
    } catch {
  ...
  const handleEditPuzzle = (puzzle: PuzzleData) => {
    ...
    setEditingOrientation(puzzle.orientation);
    setSolutionInput(puzzle.solution);
    setPuzzleDescInput(puzzle.description);
    setPuzzleHintInput(puzzle.hint || '');
    setEditingPuzzleId(puzzle.id);

    // Populate PGN textarea and auto-parse so the move history is available
    const pgn = puzzle.pgn || '';
    setPuzzlePgnText(pgn);
    setPuzzlePgnError(null);
    if (pgn) {
      try {
        const loader = new Chess();
        ...
        if (moves.length > 0) {
          ...
          setPuzzlePgnFenHistory(fenHistory);
          setPuzzlePgnMoveHistory(moves);
          setPuzzlePgnPlyIndex(fenHistory.length - 1);
        }
      } catch {
        setPuzzlePgnFenHistory([]);
        setPuzzlePgnMoveHistory([]);
  ...
  const handleBatchFetch = async () => {
    ...
    setBatchPreviews([]);
    try {
      const res = await fetch(`/api/puzzles/lichess/batch?theme=${batchTheme}&nb=${batchNb}&difficulty=${batchDifficulty}`);
      if (!res.ok) throw new Error();
      const data = await res.json();
      setBatchPreviews((data.puzzles ?? []).map((p: any) => ({
        ...p,
        editSolution: (p.solution as string[]).join(' '),
        editHint: '',
        editThemes: (p.themes as string[]).join(', '),
        editOrientation: p.orientation,
        removed: false,
      })));
    } catch { alert('Failed to fetch batch puzzles. Check that LICHESS_API_TOKEN is set.'); }
    finally { setIsBatchFetching(false); }
  };

  const handleBatchImport = () => {
    const toAdd = batchPreviews
      .filter(p => !p.removed)
      .map(p => ({
        id: generateId(),
        fen: p.fen,
        pgn: p.pgn,
        solution: p.editSolution,
        description: p.editThemes,
        hint: p.editHint,
        orientation: p.editOrientation,
      }));
    ...
    try {
      ...
      if (data.chapters?.length > 0) {
        setChapters(prev => [...prev, ...data.chapters.map((c: any, i: number) => ({
          id: generateId(), name: c.name || `Chapter ${i + 1}`,
          pgn: c.pgn || "", orientation: c.orientation || "white",
        }))]);
      } else if (data.pgn) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 min-w-0">
        ...
        <div className="rounded-lg border border-border bg-card p-4 space-y-4">
          ...
          {/* Engine Analysis — at top of details card, visible alongside board */}
          <AnalysisPanel
            fen={currentFen}
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              value={puzzlePgnText}
              onChange={e => {
                setPuzzlePgnText(e.target.value);
                setPuzzlePgnError(null);
                if (!e.target.value.trim()) { setPuzzlePgnFenHistory([]); setPuzzlePgnMoveHistory([]); }
              }}
            ...
            {puzzlePgnFenHistory.length > 0 && (
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  onClick={() => {
                    const i = Math.max(0, puzzlePgnPlyIndex - 1);
                    setPuzzlePgnPlyIndex(i);
                    handleFenChange(puzzlePgnFenHistory[i]);
                  }}
                ...
                <button
                  onClick={() => {
                    const i = Math.min(puzzlePgnFenHistory.length - 1, puzzlePgnPlyIndex + 1);
                    setPuzzlePgnPlyIndex(i);
                    handleFenChange(puzzlePgnFenHistory[i]);
                  }}
            ...
            <Button
              ...
              onClick={handleParsePuzzlePgn}
              disabled={!puzzlePgnText.trim()}
            >
              Parse PGN
            </Button>
          </div>
      ...
      {puzzles.length > 0 && (
        <div className="rounded-lg border border-border bg-card">
          ...
          <div className="px-4 py-3 border-t border-border">
            {isCompleted && createdLessonId ? (
              <SuccessBanner lessonId={createdLessonId} label="Puzzle lesson" onCreateAnother={handleCreateAnother} />
            ) : (
              <SubmitButton isSubmitting={isSubmitting} onClick={async () => {
                if (!validateLesson()) return;
                setIsSubmitting(true);
                try {
                  const id = await createPuzzleLesson(lessonInfo, puzzles, selectedStudentIds);
                  setCreatedLessonId(id); setIsCompleted(true);
                } catch (err) { alert(err instanceof Error ? err.message : "Failed to create lesson"); }
      ...
      {isLichessImportOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.45)" }}>
          <div className="w-full max-w-lg rounded-xl border border-border bg-card shadow-xl p-6 mx-4 space-y-4">
            {/* Tab toggle */}
            <div className="flex gap-1 p-1 bg-muted rounded-lg">
              {(['single', 'batch'] as const).map(tab => (
                <button key={tab} onClick={() => setImportTab(tab)}
                  className={`flex-1 text-xs py-1.5 rounded-md font-medium transition-colors ${importTab === tab ? 'bg-background shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
              ...
              <>
                <div className="grid grid-cols-3 gap-3">
                  <div className="space-y-1">
                    ...
                    <select value={batchTheme} onChange={e => setBatchTheme(e.target.value)} className="w-full h-9 rounded-md border border-input bg-background px-2 text-sm">
                      {BATCH_THEMES.map(g => (
                  ...
                  <div className="space-y-1">
                    ...
                    <select value={batchDifficulty} onChange={e => setBatchDifficulty(e.target.value)} className="w-full h-9 rounded-md border border-input bg-background px-2 text-sm">
                      {['easiest','easier','normal','harder','hardest'].map(d => (
                ...
                <Button size="sm" className="w-full" onClick={handleBatchFetch} disabled={isBatchFetching}>
                ...
                {batchPreviews.length > 0 && (
                  <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                    {batchPreviews.map((p, i) => p.removed ? null : (
                      <div key={p.lichessId} className="border border-border rounded-md p-3">
                        <div className="flex gap-3">
                          {/* Left 65% — editable fields */}
                          <div className="w-[65%] space-y-2 min-w-0">
                            <div className="flex items-center justify-between">
                              ...
                              <button onClick={() => setBatchPreviews(prev => prev.map((x, j) => j === i ? { ...x, removed: true } : x))}
                          ...
                          {/* Right 35% — mini board preview */}
                          <div className="w-[35%] flex-shrink-0">
                            <Chessboard
                              position={p.fen}
                              boardOrientation={p.editOrientation}
                ...
                <div className="flex justify-end gap-2">
                  <Button variant="outline" size="sm" onClick={closeImportModal}>Cancel</Button>
                  {batchPreviews.filter(p => !p.removed).length > 0 && (
  ...
  const renderStudyEditor = () => (
    <div className="space-y-4">
      ...
      {isCompleted && createdLessonId ? (
        <SuccessBanner lessonId={createdLessonId} label="Study lesson" onCreateAnother={handleCreateAnother} />
      ) : (
        <SubmitButton isSubmitting={isSubmitting} onClick={async () => {
          if (!validateLesson()) return;
          ...
          setIsSubmitting(true);
          try {
            const id = await createStudyLesson(lessonInfo, chapters, {
              showEval: true, showClocks: true, showArrows: true, showHighlights: true,
            }, moveAnnotations, selectedStudentIds);
            setCreatedLessonId(id); setIsCompleted(true);
            setChapters([]); setChapterNameInput(""); setPgnInput("");
            setSelectedChapterIndex(null); setMoveAnnotations(new Map());
          } catch (err) { alert(err instanceof Error ? err.message : "Failed to create lesson"); }
          finally { setIsSubmitting(false); }
        }} />
      )}
  ...
  const renderInteractiveEditor = () => (
    <div className="space-y-4">
      ...
      {isCompleted && createdLessonId ? (
        <SuccessBanner lessonId={createdLessonId} label="Interactive study" onCreateAnother={handleCreateAnother} />
      ) : (
        <SubmitButton isSubmitting={isSubmitting} onClick={async () => {
          if (!validateLesson()) return;
          if (chapters.length === 0) { alert("Please add at least one chapter"); return; }
          setIsSubmitting(true);
          try {
            const id = await createInteractiveStudyLesson(lessonInfo, chapters, {
              showEval: true, showClocks: true, showArrows: true, showHighlights: true,
            }, interactiveSolveMoves, moveAnnotations, selectedStudentIds);
            setCreatedLessonId(id); setIsCompleted(true);
            setChapters([]); setChapterNameInput(""); setPgnInput("");
            setSelectedChapterIndex(null); setMoveAnnotations(new Map());
            setInteractiveSolveMoves({});
          } catch (err) { alert(err instanceof Error ? err.message : "Failed to create lesson"); }
          finally { setIsSubmitting(false); }
        }} />
      )}
    ...
    <>
      ...
      <div className="w-full max-w-6xl mx-auto px-4 py-6 space-y-4 min-w-0">
        ...
        <div className="flex items-center justify-between gap-4 min-w-0">
          <h1 className="text-lg font-semibold truncate">Create lesson</h1>
          {selectedType && (
        ...
        {/* Lesson details — collapsible, collapsed by default */}
        {selectedType && (
          <CollapsibleSection
