...
function LoginContent() {
  ...
  return (
    <div className="min-h-dvh flex items-center justify-center p-4 bg-background">
      ...
      <Link
        href="/"
        className="fixed top-4 left-4 z-10 flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors"
      >
        <ArrowLeft className="h-4 w-4" />
        <span>Home</span>
      </Link>

      <Card className="relative w-full max-w-md shadow-lg border-border/50 rounded-sm overflow-hidden">
        {/* Pawn background inside the card */}
        <div className="absolute inset-0 pointer-events-none select-none">
          <Image
            src="/lca_pawn_light_bg.png"
            alt=""
            fill
            className="object-cover opacity-[0.18] dark:opacity-0"
            sizes="448px"
            aria-hidden
          />
          <Image
        ...
        <CardHeader className="relative z-10 flex flex-col items-center px-4 pt-6 pb-2">
          <CardTitle className="text-2xl font-bold text-foreground">Welcome Back</CardTitle>
          <CardDescription className="text-muted-foreground text-center">
            Sign in to your LCA Chess account
          </CardDescription>
        </CardHeader>
        <CardContent className="relative z-10 space-y-4 px-4 pb-6">
          {(message || error) && (
          ...
          <form onSubmit={handleEmailSubmit} className="space-y-3">
            <div className="space-y-1">
              ...
              <Input
                ...
                required
                className="w-full h-10 border-slate-300 dark:border-slate-500 dark:bg-slate-800/80 dark:text-slate-100"
                disabled={isLoading}
              />
            </div>
            ...
            <Button
              type="submit"
              className="w-full h-11 text-base font-bold rounded-sm bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white transition-colors"
              size="lg"
              disabled={isLoading}
            >
              {isLoading ? (
          ...
          <form onSubmit={handleGoogleSubmit} className="w-full">
            <Button
              type="submit"
              variant="outline"
              className="w-full h-11 text-base font-bold rounded-sm border-slate-300 bg-white text-slate-800 hover:bg-slate-50 dark:border-slate-500 dark:bg-slate-800 dark:text-slate-100 dark:hover:bg-slate-700 transition-colors"
              size="lg"
