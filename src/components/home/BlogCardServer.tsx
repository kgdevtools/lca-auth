...
export async function BlogCardServer() {
  try {
    ...
    if (!latestBlog) {
      return (
        <div className="aspect-square rounded-lg border border-border bg-card/80 dark:bg-card/60 backdrop-blur-sm p-6 flex flex-col">
    ...
    return (
      <div className="aspect-square rounded-lg border border-border bg-card/80 dark:bg-card/60 backdrop-blur-sm overflow-hidden hover:border-primary/50 transition-all duration-300 hover:shadow-lg flex flex-col">
        ...
        <div className="flex-1">
          <BlogPreviewCard
            blog={latestBlog}
            imageAlt={`Thumbnail for ${latestBlog.title}`}
          />
        </div>
      </div>
    );
    ...
    return (
      <div className="aspect-square rounded-lg border border-border bg-card/80 dark:bg-card/60 backdrop-blur-sm p-6 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center text-center">
          ...
          <Link href="/blog" className="mt-2 text-primary hover:underline">
            Try again
          </Link>
        </div>
      </div>
