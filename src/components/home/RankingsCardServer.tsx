...
export async function RankingsCardServer() {
  try {
    ...
    const filterOptions = [
      () => {
        const randomAge =
          AGE_GROUPS[Math.floor(Math.random() * AGE_GROUPS.length)];
        return {
      ...
      () => {
        ...
        return {
          ...
          label:
            randomGender === "M"
              ? "Top 10 Male Limpopo Players"
              : "Top 10 Female Limpopo Players",
        };
      },
      () => {
        return {
    ...
    const rankings =
      sorted.length === 0
        ? allRankings
            ...
            .sort(
              (a, b) =>
                (b.avg_performance_rating || 0) -
                (a.avg_performance_rating || 0),
            )
            .slice(0, 10)
        : sorted;

    const categoryLabel =
      sorted.length === 0 ? "Top 10 Players Overall" : label;

    if (rankings.length === 0) {
      return (
        <div className="aspect-square rounded-lg border border-border bg-card/80 dark:bg-card/60 backdrop-blur-sm overflow-hidden hover:border-primary/50 transition-all duration-300 hover:shadow-lg flex flex-col p-6">
    ...
    return (
      <div className="aspect-square rounded-lg border border-border bg-card/80 dark:bg-card/60 backdrop-blur-sm overflow-hidden hover:border-primary/50 transition-all duration-300 hover:shadow-lg flex flex-col">
            ...
            <table className="w-full text-xs">
              <thead className="bg-muted/50 sticky top-0 z-10">
                <tr className="border-b border-border">
                  ...
                  <th className="hidden sm:table-cell text-center p-2 font-semibold whitespace-nowrap">
                    Events
                  </th>
                  {/* Show Fed on md and up */}
                  <th className="hidden md:table-cell text-left p-2 font-semibold whitespace-nowrap">
                    Fed
                  </th>
                  {/* Show Rating on lg and up */}
                  <th className="hidden lg:table-cell text-right p-2 font-semibold whitespace-nowrap">
                    Rating
                  </th>
                  {/* APR always shown */}
    ...
    return (
      <div className="aspect-square rounded-lg border border-border bg-card/80 dark:bg-card/60 backdrop-blur-sm overflow-hidden flex flex-col p-6">
        <h2 className="text-xl font-bold mb-4 text-primary">
          Current Rankings
        </h2>
        <div className="flex-1 flex items-center justify-center text-center">
          <p className="text-muted-foreground">Error loading rankings</p>
          <Link href="/rankings" className="mt-2 text-primary hover:underline">
            Try again
          </Link>
        </div>
      </div>
