/**
 * Framework-agnostic, browser-safe port of src/scripts/rank-by-performance.ts.
 *
 * `rankPlayers()` takes the raw view rows + tournaments + region mapping and a
 * filter object, and returns ranked players (with per-tournament appearances)
 * exactly as the terminal script does — grouping precedence, confidence gating,
 * averaging, and all filters. No I/O, no Node APIs — safe in server or client
 * components. Render the result with your shadcn/ui components.
 */
...
export function rankPlayers(
  ...
  for (const r of appearances) {
    ...
    // Appearance-level filters.
    if (feds.length && !(r.federation && feds.includes(r.federation.toUpperCase()))) continue;
    ...
    const perf = num(r.performance_rating);
  ...
  for (const p of players.values()) {
    ...
    const seeds = p.appearances.map((a) => a.seed).filter((v): v is number => v !== null);
